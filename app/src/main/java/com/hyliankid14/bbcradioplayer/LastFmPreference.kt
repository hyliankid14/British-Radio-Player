...
object LastFmPreference {
    ...
    // Official Last.fm API key and secret registered for British Radio Player (hyliankid14)
    const val DEFAULT_API_KEY = "fb00b9b8b3289361e41d8cd439f9bf81"
    const val DEFAULT_API_SECRET = "31150c97024adb24e569c7b30efd7492"
    ...
    fun getEffectiveApiKey(context: Context): String {
        ...
        if (BuildConfig.LASTFM_API_KEY.isNotBlank()) return BuildConfig.LASTFM_API_KEY
    ...
    fun getEffectiveApiSecret(context: Context): String {
        ...
        if (!custom.isNullOrEmpty()) return custom
        if (BuildConfig.LASTFM_API_SECRET.isNotBlank()) return BuildConfig.LASTFM_API_SECRET
