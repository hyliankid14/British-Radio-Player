...
/**
 * Central coordinator for Wear OS track scrobbling to Last.fm and external scrobblers.
 ...
 */
object WearScrobbleManager {
    private const val TAG = "WearScrobbleManager"
    ...
    @Synchronized
    fun updateTrackDuration(durationSec: Int) {
        val current = activeTrack ?: return
        if (durationSec > 0 && current.durationSec <= 0) {
            current.durationSec = durationSec
            Log.d(TAG, "Updated track duration: ${current.artist} - ${current.track} (${durationSec}s)")
        }
    }
    ...
    fun onTrackStarted(
        ...
        if (isPodcast && !WearLastFmPreference.shouldScrobblePodcasts(context)) {
            Log.d(TAG, "Ignoring podcast track ($trimmedTrack) as podcast scrobbling is disabled on Wear")
            return
        }
        ...
        Log.d(TAG, "New watch track started: ${newTrack.artist} - ${newTrack.track} (duration: ${durationSec}s)")

        // 1. Broadcast standard scrobbler intent
        WearScrobbleBroadcastManager.broadcastPlayState(
            context,
            WearScrobbleBroadcastManager.SLS_STATE_START,
            newTrack.artist,
            newTrack.track,
            newTrack.album,
            durationSec
    ...
    fun onPlaybackPaused(context: Context) {
        ...
        if (current.isPlaying) {
            ...
            WearScrobbleBroadcastManager.broadcastPlayState(
                context,
                WearScrobbleBroadcastManager.SLS_STATE_PAUSE,
                current.artist,
                current.track,
                current.album,
                current.durationSec
    ...
    fun onPlaybackResumed(context: Context) {
        ...
        if (!current.isPlaying) {
            ...
            WearScrobbleBroadcastManager.broadcastPlayState(
                context,
                WearScrobbleBroadcastManager.SLS_STATE_RESUME,
                current.artist,
                current.track,
                current.album,
                current.durationSec
    ...
    fun onPlaybackStopped(context: Context) {
        ...
        if (current != null) {
            WearScrobbleBroadcastManager.broadcastPlayState(
                context,
                WearScrobbleBroadcastManager.SLS_STATE_COMPLETE,
                current.artist,
                current.track,
                current.album,
                current.durationSec
    ...
    private fun scheduleDirectScrobble(context: Context, track: ActiveTrack) {
        ...
        scope.launch {
            ...
            if (WearLastFmPreference.isDirectScrobbleEnabled(context)) {
                WearLastFmApiClient.updateNowPlaying(
                    context = context,
                    artist = track.artist,
                    track = track.track,
                    album = track.album,
            ...
            synchronized(this@WearScrobbleManager) {
    ...
    private fun triggerScrobble(context: Context, track: ActiveTrack) {
        ...
        Log.i(TAG, "Scrobbling track from Wear: ${track.artist} - ${track.track} (listened ${track.totalListenedMs}ms)")

        if (WearLastFmPreference.isDirectScrobbleEnabled(context)) {
            scope.launch {
                val timestampSec = track.startTimeMs / 1000L
                WearLastFmApiClient.scrobble(
                    context = context,
                    artist = track.artist,
                    track = track.track,
